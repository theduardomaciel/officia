...
export default function CategoriesScreen({ route }: any) {
	...
	const [categoryToEditId, setCategoryToEditId] = React.useState<
		string | undefined
	>(undefined);
	...
	return (
		<BusinessLayout
			headerProps={{
				title: "Categorias",
				description:
					"Adicione, gerencie e organize as categorias de seu negócio com facilidade para melhorar a eficiência da gestão de pedidos e agendamentos.",
			}}
		...
		>
			...
			<BottomSheet id={"categoriesBottomSheet"} height={"49%"}>
				<CategoryForm
...
interface CategoryFormProps {
	...
	setCategories: React.Dispatch<
		React.SetStateAction<BusinessData["categories"]>
	>;
...
function CategoryForm({
	...
	const [newCategoryColor, setNewCategoryColor] = React.useState<string>(
		colors.primary
	);
	...
	async function handleCategory() {
		if (
			cachedCategoryName.current.length > 0 ||
				...
				setCategories(
					(prev) =>
						...
						prev.map((oldCategory) =>
							oldCategory.id === categoryToEditId
								? {
										...oldCategory,
	...
	useEffect(() => {
		if (categoryToEditId) {
			const categoryToEdit =
				categories &&
				categories.find((category) => category.id === categoryToEditId);
			if (categoryToEdit) {
		...
		<>
			...
			>
				<Input
					...
					onEndEditing={() =>
						setNewCategoryName(cachedCategoryName.current)
					}
				...
				>
					<View style={{ flex: 1 }}>
						<Label style={{ marginRight: 10 }}>Cor</Label>
				...
				>
					<View style={{ flex: 1 }}>
						<Label style={{ marginRight: 10 }}>Ícone</Label>
						<Text className="text-sm leading-4 text-text-200">
					...
					<View className="w-10 h-10 flex items-center justify-center">
						<MaterialIcons
							name={newCategoryIcon as unknown as any}
							size={32}
							color={colors.white}
						/>
					</View>
			...
			>
				<View className="flex-col w-full mt-4">
					<ColorPicker
						...
						value={newCategoryColor ?? colors.primary}
						onComplete={onSelectColor}
					>
						<Panel3 style={{ width: "100%" }} />
						<BrightnessSlider style={{ minWidth: "100%" }} />
						<Swatches
							swatchStyle={{ shadowOpacity: 0 }}
...
const TagPreview = ({
	...
	return (
		...
		>
			...
			>
				<MaterialIcons
					name={tag.icon as unknown as any}
					size={20}
					color={colors.white}
				/>
