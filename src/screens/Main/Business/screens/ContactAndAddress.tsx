...
export function ContactAndAddress({
	businessData,
	control,
	errors,
	onAddressFetch,
}: FormProps & ContactAndAddressProps) {
	const decodedGeocodedAddress = businessData.geocodedAddress
		? businessData.geocodedAddress.split(", ").map((geo) => geo)
		: undefined;
	...
	return (
		<BusinessScrollView>
			...
			/>
			<SubSectionWrapper
				header={{ title: "Endereço", icon: "location-on" }}
			>
				...
				{businessData.geocodedAddress ? (
					<View className="flex-col w-full" style={{ rowGap: 15 }}>
						<Input
							label="Logradouro"
							value={
								decodedGeocodedAddress &&
								decodedGeocodedAddress[0]
							}
							pallette="disabled"
						/>
						<Controller
...
export default function ContactAndAddressScreen({ route }: any) {
	...
	const onError: SubmitErrorHandler<ContactAndAddressSchemeType> = (
		errors,
		e
	) => {
		//console.log(errors)
