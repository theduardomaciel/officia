...
const ToastUI = forwardRef(
	(
		...
		const parsedHeight =
			(parseFloat(toastOffset.split("%")[0]) / 100) * screenHeight;
		const activeHeight = useSharedValue(
			toastPosition === "top" ? screenHeight - parsedHeight : parsedHeight
		);
		const newActiveHeight = screenHeight - activeHeight.value;

		const topAnimation = useSharedValue(
			toastPosition === "top" ? -screenHeight : screenHeight
		);
		const animationStyle = useAnimatedStyle(() => {
			const compensation =
				contentHeight.value > 0 ? contentHeight.value / 2 : 0;
			const top = topAnimation.value - compensation; // removemos metade do tamanho do conteúdo para que o conteúdo fique centralizado
			const display =
				top === screenHeight - compensation ||
				top === -screenHeight - compensation
					? "none"
					: "flex";
			...
			onActive: (event, ctx: any) => {
				...
				if (
					...
					((toastPosition === "bottom" &&
						event.translationY < -maxDragDistance) ||
						(toastPosition === "top" &&
							event.translationY > maxDragDistance))
				) {
		...
		const hide = useCallback((direction: "top" | "bottom") => {
			...
			topAnimation.value = withSpring(
				direction === "bottom" ? screenHeight : -screenHeight,
				animProps
			);
		}, []);
							...
							<View className="flex-col items-start">
								<View className="mb-2">
									{toastProps?.preset === "error" ? (
										...
										<MaterialIcons
											name={
												toastProps?.icon as unknown as any
											}
											size={24}
											color={colors.white}
										/>
								...
								<Text
									className="font-titleBold text-lg mb-1 text-black dark:text-white"
									style={{
										lineHeight: 20,
									}}
								>
