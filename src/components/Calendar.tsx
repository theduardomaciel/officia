...
export function WeekDays({ invert }: { invert?: boolean }) {
	return (
		<View
			className={`flex flex-row items-center justify-between w-full px-[17px]`}
		>
...
function DayView({
	...
	return (
		<TouchableOpacity
			className={clsx(
				"flex flex-1 rounded-full aspect-square items-center justify-center p-1 bg-gray_light-neutral bg-black dark:bg-gray-200 border-white",
				{
					"bg-white dark:bg-gray-200": invert,
				}
			)}
			style={[
				{ borderWidth: selected ? 1 : 0, ...style },
		...
		>
			...
			{status && status === "busy" ? (
				...
				<View className="absolute bottom-[5px] rounded-full w-[3.5px] h-[3.5px] bg-primary" />
			) : null}
		</TouchableOpacity>
	);
}
...
function getMonthInfo(currentMonth: number, currentDate: Date) {
	...
	let monthDates = Array.from(
		{ length: lastDateOfMonth - firstDateOfMonth + 1 },
		(_, index) => {
			return {
	...
	if (remainingDaysOnLastMonth > 0) {
		...
		const remainingPreviousMonthDates = Array.from(
			{ length: remainingDaysOnLastMonth },
			(_, index) => {
				return {
	...
	if (remainingDaysOnNextMonth > 0) {
		...
		const remainingNextMonthDates = Array.from(
			{ length: remainingDaysOnNextMonth },
			(_, index) => {
				return {
		...
		monthDates = monthDates
			.concat(remainingNextMonthDates)
			.splice(0, daysAmountOnCalendar);
	}
...
export default function Calendar({
	...
	const Header = () => (
		<Animated.Text
			style={animatedStyle}
			className="text-white w-3/4 text-center font-titleBold text-xl"
		>
			{MONTHS[currentMonth]}
		</Animated.Text>
	);
	...
	return (
		<View
			className="flex-col w-full bg-black dark:bg-gray-500 rounded-xl"
			style={style ? style : { padding: 16 }}
		>
			<View className="flex-row items-center justify-around mb-2">
				<MaterialIcons
					...
					onPress={() =>
						currentMonth > 0 &&
						setCurrentMonth(
							(actualStateMonth) => actualStateMonth - 1
						)
					}
				/>
				{isStatic ? (
			...
			<FlatList
				...
				renderItem={({ item, index }) => {
					...
					const daysBeforeToday =
						currentDate.getDate() + firstDayOfMonth - 1;

					//console.log(currentMonth, selectedDate?.month, "currentMonth, selectedDate?.month")

					const REMAINING_LAST =
						(remainingDaysOnLastMonth > 0 &&
							index < remainingDaysOnLastMonth) ||
						(isStatic && currentMonth < currentDate.getMonth());
					const REMAINING_NEXT =
						remainingDaysOnNextMonth > 0 &&
						index >= monthDates.length - remainingDaysOnNextMonth;
					...
					return (
						<DayView
							...
							selectedPreset={
								selectedDate && isEqualToSelected
									? "dashed_green"
									: undefined
							}
							...
							status={
								statusArray && statusArray[currentMonth][index]
							}
							invert
							activeOpacity={REMAINING_LAST && isStatic ? 1 : 0.5}
							style={{
								opacity:
									REMAINING_LAST || REMAINING_NEXT ? 0.5 : 1,
								marginBottom: 5,
								marginRight: 5,
								width: 35,
								height: 35,
