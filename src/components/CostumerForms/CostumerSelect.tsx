...
export default function ClientSelect({ lastBottomSheet, order }: Props) {
    ...
    async function handleSelectClient(client: CostumerModel) {
...
async function deleteClient(client: CostumerModel) {
    await database.write(async () => {
...
const EnhancedClientPreview = ({ client, ...rest }: any) => {
    const [observedClient, setClient] = useState<CostumerModel>(client);

    useEffect(() => {
        client.observe().subscribe((client: CostumerModel) => {
            setClient(client);
        });
    }, []);
