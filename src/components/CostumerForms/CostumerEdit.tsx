...
import { ClientDeleteModal } from "./CostumerSelect";
...
export default function ClientEdit({ order, lastBottomSheet, client }: Props) {
    ...
    async function handleUpdate(updatedClient: CostumerModel) {
        try {
    ...
    const onSubmit: SubmitHandler<ClientFormValues> = (data) => {
        ...
        handleUpdate(updatedClient as unknown as CostumerModel);
        reset();
